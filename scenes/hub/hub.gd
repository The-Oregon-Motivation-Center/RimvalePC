...
func _refresh_party_list() -> void:
	...
	for handle in GameState.collection:
...
func _on_add_character() -> void:
	if GameState.collection.size() >= GameState.ACTIVE_TEAM_SIZE:
		# Flash a warning in the detail panel
		_clear_detail()
		var lbl = Label.new()
		lbl.text = "Party is full! (max %d)" % GameState.ACTIVE_TEAM_SIZE
		lbl.add_theme_font_size_override("font_size", 18)
...
func _on_remove_character(handle: int) -> void:
	...
	GameState.remove_from_collection(handle)
	_selected_handle = -1
	_show_detail_placeholder()
	_refresh_party_list()
